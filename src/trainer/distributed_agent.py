...
class DistributedAgent():
        ...
        if not self.__local_run:
            ...
            while True:
                ...
                try:
                    ...
                    response = requests.get('http://{0}:7777/ping'.format(self.__possible_ip_addresses[ping_idx % len(self.__possible_ip_addresses)])).json()
    ...
    def __publish_batch_and_update_model(self, batches, batches_count):
        ...
        if not self.__local_run:
            ...
            response = requests.post('http://{0}:7777/gradient_update'.format(self.__trainer_ip_address), json=post_data)
            print('Response:')
            print(response)
    ...
    def __get_latest_model(self):
        ...
        response = requests.get('http://{0}:7777/latest'.format(self.__trainer_ip_address)).json()
...
def setup_logs(parameters):
    output_dir = os.path.join(parameters['data_dir'], 'logs\\{0}\\agent'.format(parameters['experiment_name']))
